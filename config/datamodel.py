...
#   timestamp: 2025-10-02T22:12:14+00:00
...
class VisionConfig(BaseModel):
    detail: Optional[Detail] = Field(
        "low",
        description="Image detail level (low=512px max/faster, high=2048px max/slower, auto=adaptive)",
    )
    ...
    max_image_size_kb: Optional[int] = Field(
        500, description="Warn if image exceeds this size in KB"
    )
    resize_images: Optional[bool] = Field(
        False, description="Automatically resize images (future feature)"
    )
    max_dimension: Optional[int] = Field(
        512, description="Maximum width/height when resizing (future feature)"
    )
...
class Model(BaseModel):
    ...
    vision_config: Optional[VisionConfig] = Field(
        None, description="Vision-specific configuration for image processing"
    )
    lemonade: Optional[Lemonade] = Field(None, description="Lemonade-specific configuration")
