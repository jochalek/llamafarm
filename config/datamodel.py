...
#   timestamp: 2025-10-03T00:26:42+00:00
...
class Parser(BaseModel):
    ...
    priority: Optional[conint(ge=0, le=1000)] = Field(
        50, description="Parser priority (lower = try first)"
    )
...
class Extractor(BaseModel):
    ...
    priority: Optional[conint(ge=0, le=1000)] = Field(
        50, description="Extractor priority (lower = apply first)"
    )
...
class VisionConfig(BaseModel):
    max_tokens: Optional[int] = Field(512, description="Maximum tokens for vision responses")
    temperature: Optional[float] = Field(
        0.1, description="Temperature for vision model (lower=more focused)"
    )
    timeout: Optional[float] = Field(
        300, description="Request timeout in seconds (vision models can be slow, 300s = 5 minutes)"
    )
    max_image_size_kb: Optional[int] = Field(
        500, description="Maximum image size in KB before resizing (when resize_images is true)"
    )
    resize_images: Optional[bool] = Field(
        True, description="Automatically resize images larger than max_image_size_kb"
    )
    max_dimension: Optional[int] = Field(
        768, description="Maximum width/height in pixels when resizing images"
    )
...
class ModelInfo(BaseModel):
    checkpoint: Optional[str] = Field(
        None,
        description="HuggingFace checkpoint (e.g., 'google/gemma-3-4b-it-qat-q4_0-gguf')",
        examples=["google/gemma-3-4b-it-qat-q4_0-gguf"],
    )
    variant: Optional[str] = Field(
        None,
        description="Variant/quantization for GGUF models (e.g., 'Q4_0', 'Q8_0')",
        examples=["Q4_0"],
    )
    recipe: Optional[Recipe] = Field(
        None, description="Recipe/backend for Lemonade models", examples=["llamacpp"]
    )
    reasoning: Optional[bool] = Field(
        False, description="Whether this is a reasoning model (Lemonade)"
    )
    mmproj_file: Optional[str] = Field(
        None, description="Multimodal projector file name for vision models (Lemonade)"
    )
...
class Lemonade(BaseModel):
    backend: Optional[Backend] = Field("onnx", description="Inference backend for Lemonade")
    port: Optional[int] = Field(11534, description="Port for Lemonade server")
    context_size: Optional[int] = Field(32768, description="Context window size in tokens")
    model_path: Optional[str] = Field(None, description="Custom model storage location")


class Model(BaseModel):
    name: str = Field(..., description="Model identifier (unique name)")
    description: Optional[str] = Field(
        None, description="Human-readable description of this model configuration"
    )
    ...
    huggingface_token: Optional[str] = Field(
        None, description="Hugging Face API token for downloading gated models"
    )
    instructor_mode: Optional[str] = Field(
        None,
        description="Instructor mode to use for structured output (e.g., tools, json, md_json)",
    )
    prompt_format: Optional[PromptFormat] = Field(
        None,
        description="Prompt formatting pipeline (structured uses Instructor; unstructured sends freeform prompts; image for vision/multimodal)",
    )
    ...
    vision: Optional[bool] = Field(
        False, description="Whether this model supports vision/image inputs"
    )
    mmproj: Optional[str] = Field(
        None, description="Path to multimodal projector file (for llamacpp vision models)"
    )
    vision_config: Optional[VisionConfig] = Field(
        None, description="Vision-specific configuration for image processing and optimization"
    )
    model_info: Optional[ModelInfo] = Field(
        None, description="Model pull/download information (used by 'lf models pull' command)"
    )
    lemonade: Optional[Lemonade] = Field(
        None, description="Lemonade-specific runtime configuration"
    )
...
class Runtime(BaseModel):
    ...
    models: Optional[list[Model]] = Field(
        None, description="List of model configurations for multi-model support"
    )
