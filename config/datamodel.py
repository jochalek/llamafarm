...
#   timestamp: 2025-10-08T04:08:26+00:00
...
class EmbeddingStrategy(BaseModel):
    ...
    config: dict[str, Any] = Field(..., description="Embedder configuration")
...
class RetrievalStrategy(BaseModel):
    ...
    config: dict[str, Any] = Field(..., description="Retrieval configuration")
...
class Parser(BaseModel):
    ...
    type: Type3 = Field(..., description="Parser type")
    config: dict[str, Any] = Field(..., description="Parser configuration")
...
class Extractor(BaseModel):
    ...
    config: dict[str, Any] = Field(..., description="Extractor configuration")
...
class Agent(BaseModel):
    name: str = Field(..., description="Agent identifier (unique name)")
    type: Type4 = Field(
        ..., description="Agent type (chat_orchestrator, document_analyzer, rag_validator, custom)"
    )
    model: Optional[str] = Field(
        None, description="Model name from runtime.models to use (defaults to default_model)"
    )
    description: Optional[str] = Field(None, description="Human-readable description")
    system_prompt: Optional[str] = Field(None, description="Override system prompt for this agent")
    tools: Optional[list[str]] = Field(None, description="List of tool names this agent can use")
    prompt_format: Optional[PromptFormat] = Field(None, description="Override prompt format")
...
class LlamaFarmConfig(BaseModel):
    ...
    agents: Optional[list[Agent]] = Field(
        [], description="Agent configurations for orchestration and workflows"
    )
